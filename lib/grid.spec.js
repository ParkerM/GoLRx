import { Grid, OOB_CELL } from './grid.js';
...
describe('Grid', () => {
  ...
  xdescribe('getCellAt', () => {
    it('returns static oob marker cell if reaching off grid', () => {
      // given grid G(x,y) s.t. x = {0, 1}, y = {0, 1, 2, 3}
      const grid = new Grid(2, 4);
      // for any cell C = (x_c, y_c)
      // C nin G for x_c < 0
      expect(grid.getCellAt(-1, 1)).toBe(OOB_CELL);
      // C nin G for y_c < 0
      expect(grid.getCellAt(1, -1)).toBe(OOB_CELL);
      // C nin G for x_c > 1
      expect(grid.getCellAt(3, 1)).toBe(OOB_CELL);
      // C nin G for y_c > 3
      expect(grid.getCellAt(1, 3)).toBe(OOB_CELL);
      // C in G for x_c = 1, y_c = 1
      expect(grid.getCellAt(1, 1)).not.toBe(OOB_CELL);
    });
  });
});
